		...
		if(m_nSelectedIconIndex > -1 && m_nSelectedIconIndex < (int)m_uIconsCount)
		{
			...
			if(m_pIconData[m_nSelectedIconIndex]->GetToolTip() != NULL)
			{
				...
				if (m_pIconData[m_nSelectedIconIndex]->GetToolTip()) 
					lstrcpyn(m_tNotifyIconData.szTip, (CString)m_pIconData[m_nSelectedIconIndex]->GetToolTip(), sizeof(m_tNotifyIconData.szTip)); 
				else 
...
BOOL CTrayIcon::HoverIcon()
{
	if( m_uHoverTimer == 0 )
	{
		...
		m_uHoverTimer = (UINT)SetTimer(HOVER_EVENT, SECOND, 0);
	}
...
BOOL CTrayIcon::StartIconAnimation()
{
	...
	{
		m_uTimer = (UINT)SetTimer(ANIMATE_EVENT, m_uElapse, 0);
		if(m_uTimer > 0)
		{
			m_bAnimate = true;
			return true;
